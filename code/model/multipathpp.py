...
class MultiPathPP(nn.Module):
    ...
    def forward(self, data, num_steps):
        target_scatter_numbers = torch.ones(data["batch_size"], dtype=torch.long).cuda() #(1,42), all element 1
        target_scatter_idx = torch.arange(data["batch_size"], dtype=torch.long).cuda() #(1,42), element 0-41
        ...
        #print(target_agents_embeddings.shape)
        ...
        #print(interaction_agents_embeddings.shape)
